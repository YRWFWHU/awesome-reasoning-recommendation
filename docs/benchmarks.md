...
### 偏好画像的变化不保证排序响应（2026-09-21补充）

[UPR画像干预研究](../README.md#paper-transparent-upr)在所测评分回归和候选协议下检查反事实文本与激活干预，未检测到显著类型选择性排序变化。它检验的是中间偏好证据能否控制决策，不是CoT总体有效性的判断。[固定v1 §5.4](https://arxiv.org/html/2609.19831v1)

**记录建议（维护者）：** 对照画像内容、表示变化、评分整体偏移和目标类别排序响应；同时保留训练目标与候选协议，避免只凭表示可视化宣称决策可控。
