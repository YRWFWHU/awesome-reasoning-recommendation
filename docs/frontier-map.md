...
- **中间表示是否真的改变排序：** [UPR画像干预研究](../README.md#paper-transparent-upr)提供所测评分回归协议下的负面证据；[NarraLite](../README.md#paper-narralite)则用未来语义对齐潜在表示。维护者建议：分开检验表示可解释性、标签预测收益和干预后的决策变化，不能互相替代。
- **训练计算与在线搜索：** [TGR](../README.md#paper-tgr)关闭推理rollout后导出SID先验，[LIGE-GR](../README.md#paper-lige-gr)在线估计未来价值并搜索列表。维护者建议：分别报告训练、离线刷新、在线解码预算，并隔离搜索宽度与价值目标的影响。

以上为维护者归纳，不是新颖性保证；固定版本与成本未知项见[周检记录](../notes/weekly-search-2026-09-21.md)。
